...
@section('content')
    <div class="row">
        <div class="col-sm-12">
            <div class="card-box">
                <div class="row">
                    <form action="{{route('roles.update', $data->id)}}" method="post" role="form">
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="form-horizontal">
                                <div class="form-group">
                                    ...
                                    <div class="col-md-6">
                                        <input type="text" id="nameForm" name="name" class="form-control" placeholder="Slug name" value="{{ $data->name }}">
                                    </div>
                                </div>
                                <div class="form-group">
                                    <label class="col-md-3 control-label" for="displayNameForm">Display Name</label>
                                    <div class="col-md-6">
                                        <input type="text" id="displayNameForm" name="title" class="form-control" placeholder="..." value="{{ $data->title }}">
                                    </div>
                                </div>
                                <div class="form-group">
                                    ...
                                    <div class="col-md-6">
                                        <select name="level" id="levelForm" class="form-control" required>
                                            ...
                                            <option value="{{ $level->id }}" {{ $level->id == $data->level ? "selected" : "" }}>{{ $level->name }}</option>
                                            @empty
                        ...
                        <div class="col-md-6">
                            ...
                            <select multiple="multiple" class="multi-select" name="permission[]" data-plugin="multiselect" data-selectable-optgroup="true">
                                @forelse ($abilities as $item)
                                    <option 
                                    value="{{ $item->name }}"
                                    {{ in_array($item->id, $plucks) ? "selected" : "" }} >{{ $item->name }}</option>
                                @empty
                                    <option value="">No data</option>
                                @endforelse
                            </select>
